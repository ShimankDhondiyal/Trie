...
public class TrieApp {
	...
	throws IOException {
		...
		String[] allWords = new String[numWords];	//make string array with size of max words
		for (int i=0; i < allWords.length; i++) {
			allWords[i] = sc.nextLine().trim().toLowerCase();	//trim and make lowercase
		}
		sc.close();
		//allwords is an array of size (given on line 1) containing all strings
		// build Trie
